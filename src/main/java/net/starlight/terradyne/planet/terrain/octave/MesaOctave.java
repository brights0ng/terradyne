...
public class MesaOctave implements IUnifiedOctave {
    ...
    public double generateHeightContribution(int x, int z, OctaveContext context,
        ...
        // === STEP 7: APPLY MESA MASK - ONLY ADD HEIGHT WHERE MESAS SHOULD BE ===
        // The key fix: only return significant height where plateauMask is substantial
        if (plateauMask < 0.1) {
            // No mesa here - return minimal contribution so rolling terrain shows
            return pedestals; // Only small pedestals, if any
        }

        // Mesa area - combine all mesa elements
        ...
        // === STEP 9: SMOOTH TRANSITION AT MESA EDGES ===
        // Blend mesa contribution based on plateau mask strength
